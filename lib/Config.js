...
exports.Config = koishi_1.Schema.intersect([
    ...
    koishi_1.Schema.object({
        keyWordType: koishi_1.Schema.union(['chatgpt', 'tencent'])
        ...
        keyWordLength: koishi_1.Schema.number().default(3).description('关键词的个数')
            .min(1).max(5).step(1),
        keyWordKey: koishi_1.Schema.object({ secretId: String, secretKey: String }).role('secret')
            .default({
            secretId: '',
            secretKey: '',
        })
    ...
    koishi_1.Schema.object({
        isUseSearch: koishi_1.Schema.boolean().default(true).description('是否使用搜索引擎'),
        searchNumber: koishi_1.Schema.number().default(1).description('搜索数据的条数')
            .min(1).max(3).step(1),
        searchType: koishi_1.Schema.union(['bing', 'baidu']).default('bing').description('搜索引擎的类型，目前只支持bing和百度，以后会考虑增加其他的搜索引擎'),
    }).description('搜索 设置'),
    koishi_1.Schema.object({
